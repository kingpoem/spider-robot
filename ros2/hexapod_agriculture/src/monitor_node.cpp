...
public:
    MonitorNode() : Node("monitor_node")
    {
        ...
        monitor_data_pub_ = this->create_publisher<std_msgs::msg::String>("monitor/data", 10);
        status_pub_ = this->create_publisher<std_msgs::msg::String>("monitor/status", 10);
        
        monitoring_ = false;
        
        RCLCPP_INFO(this->get_logger(), "山核桃病害监测节点已启动");
    }
...
private:
    ...
    void startMonitoring()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "开始山核桃病害监测");
    }
    
    void processMonitoringData(const sensor_msgs::msg::Image::ConstSharedPtr msg)
    {
        std::string disease_status = detectPecanDisease(msg);
        
        auto data = std_msgs::msg::String();
        data.data = "山核桃病害监测: " + disease_status + ", 图像=" + 
                   std::to_string(msg->width) + "x" + std::to_string(msg->height);
        monitor_data_pub_->publish(data);
    }
    
    std::string detectPecanDisease(const sensor_msgs::msg::Image::ConstSharedPtr /* msg */)
    {
        return "正常";
    }
